...
export default ({}) => {
	...
	const fetchPersonnes = (querySearch) => {
		const query = !!querySearch
			? querySearch
			: `size=${rowsPerPage}&page=${page}&cin=${cin}&nom=${nom}&prenom=${prenom}&poste=${post}`;
		setIsLoading(true);
							...
							>
								...
								>
									...
									>
										...
										<em>
											{(!!singleUserData && singleUserData?.personnel?.nom) +
												" " +
												(!!singleUserData &&
													singleUserData?.personnel?.prenom)}{" "}
										</em>
										.
									</p>
								...
								<div>
									...
									>
										<Avatar sx={{ bgcolor: Colors.secondary }}>
											{!!singleUserData &&
												singleUserData?.personnel?.nom.slice(0, 2)}
										</Avatar>
										<p
											style={{
												fontFamily: Fonts().primaryRegular,
